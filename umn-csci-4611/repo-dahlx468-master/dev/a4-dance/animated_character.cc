                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    /** TODO: You will need to define a current transformation matrix for this bone that takes into account not just the parent_transform but also the local rotation of the bone due to the current pose.
        ...
        Think of the vertices that make up the geometry of each bone as being defined in "bone space", where the joint that the bone rotates around is located at the origin and the bone extends 
        
        in the direction and length specified by the skeleton. (See Skeleton::BoneDirectionAndLength()).
     
        To determine which matrices need to be composed to create the current transformation matrix and the order to multiply them together, 
        
        think about what needs to happen to each vertex of a cylinder defined in "bone space" in order to get the vertex to the correct position in 3D space.
     
        First, the vertex must be transformed into the bone's "rotation axis space" because the rotation axes are not guaranteed to line up perfectly with the bone's X,Y,Z axes.  
        
        The bone's rotation axes are a property of the skeleton -- they are set for each skeleton and do not change for each pose.  You can access a matrix that transforms from "bone space" to 
        
        "rotation axis space" from the skeleton_ member variable.
        ...
        Third, with the rotations applied relative to the appropriate rotation axes, the vertices must now be transformed back into regular "bone space".  At this point, the bone should be properly 
        
        rotated based upon the current pose, but the vertices are still defined in "bone space" so they are close to the origin.
    ...
    */
    ...
    ctm = parent_transform * skeleton_.BoneSpaceToRotAxesSpace(bone_name) * pose_.JointRotation(bone_name) * skeleton_.RotAxesSpaceToBoneSpace(bone_name);
    ...
    if (bone_name == "lowerback") {
        Matrix4 e, p;
        e = Matrix4::Translation(Vector3(0, -0.05, 0)) * Matrix4::RotationX(20 * M_PI / 180) * Matrix4::Scale(Vector3(0.125, 0.25, 0.125));
        quick_shapes_.DrawSphere(ctm * e, view_matrix, proj_matrix, Color(0, 0, 0));
    }
    if (bone_name == "upperback") {
        Matrix4 e;
        e = Matrix4::Translation(skeleton_.BoneDirectionAndLength(bone_name)) * Matrix4::Scale(Vector3(0.1, 0.1, 0.1));
        quick_shapes_.DrawSphere(ctm * e, view_matrix, proj_matrix, Color(0, 0, 0));
    }
    if (bone_name == "thorax") {
        Matrix4 e;
    ...
    if (bone_name == "head") {
        Matrix4 e, head, leye, reye, lscape, rscape;
        e = Matrix4::RotationX(-25 * M_PI / 180) * Matrix4::Scale(Vector3(0.1, 0.15, 0.1));
        head = e * Matrix4::Translation(Vector3(0, 0, 0.5));
        leye = Matrix4::Translation(Vector3(-0.03, 0.075, 0.125)) * Matrix4::RotationX(45 * M_PI / 180) * Matrix4::Scale(Vector3(0.02, 0.02, 0.02));
        reye = Matrix4::Translation(Vector3(0.03, 0.075, 0.125)) * Matrix4::RotationX(45 * M_PI / 180) * Matrix4::Scale(Vector3(0.02, 0.02, 0.02));
        lscape = e * Matrix4::Translation(Vector3(-0.6, 1.5, 0.5)) * Matrix4::RotationZ(10 * M_PI / 180) * Matrix4::Scale(Vector3(0.25, 0.75, 0.25));
        rscape = e * Matrix4::Translation(Vector3(0.6, 1.5, 0.5)) * Matrix4::RotationZ(-10 * M_PI / 180) * Matrix4::Scale(Vector3(0.25, 0.75, 0.25));

        quick_shapes_.DrawSphere(ctm * head, view_matrix, proj_matrix, Color(0, 0, 0));
        quick_shapes_.DrawSphere(ctm * leye, view_matrix, proj_matrix, Color(0, 0, 0));
        quick_shapes_.DrawSphere(ctm * reye, view_matrix, proj_matrix, Color(0, 0, 0));
        quick_shapes_.DrawSphere(ctm * lscape, view_matrix, proj_matrix, Color(0, 0, 0));
        quick_shapes_.DrawSphere(ctm * rscape, view_matrix, proj_matrix, Color(0, 0, 0));
    }
