                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    //First and last points of the stroke
    Point3 first;
    Point3 last;
    Vector3 planeVector;
    Vector3 normalVector;

    //bool ScreenPtHitsGround(const Matrix4 &view_matrix, const Matrix4 &proj_matrix, const Point2& normalized_screen_pt, Point3* ground_point);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), &first);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), &last);

    planeVector = first - last;
    //create plane that points towards camera and id parallel to the ground plane, can use cross of 0, 1, 0 and normalize
    normalVector = planeVector.Cross(Vector3(0, 1, 0));
